...
- [ ] **S6** docs の `LD_PRELOAD` 言及を「移行完了」に更新
  - EC2 で `~/sensor_demo` 単体起動、Button17 → LED18、RFID tap → UID ログまで確認済み
...
- [x] **G4** GPIO の `LD_PRELOAD=gpio_shim.so` を起動手順から削除 — 2026-06-03。gpio-sim + GPIO chardev v2 経路で `~/sensor_demo` 単体起動確認済み
...
- [x] **S5** `LD_PRELOAD=spi_shim.so` を削除、起動コマンドを `~/sensor_demo` のみに（S4 の実機確認が取れ次第） — 2026-06-03。LD_PRELOAD なしの `~/sensor_demo` で GPIO/I2C/SPI 経路確認済み
