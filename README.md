...
```powershell
# Windows (USB / adb 経路)
agp native sync                                  # Codespace から取得して sensor_demo を adb push
adb shell
```

adb 実機が WSL2 に見えていない場合、`agp native sync` / `agp native deploy` は `agp usb attach` 相当を自動で先行実行します。
...
agp native sync --host raspi5                    # Codespace から取得して scp 配送
