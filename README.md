...
```bash
make init
make start                                     # EC2 既定 host も .agp/config.json に保存
```
...
cd repos/embedded-poc-app && make                 # target app ビルド (aarch64)
cd ../agp-tools && make                           # simulation tools ビルド (aarch64)
```
```bash
# WSL hub
cd ~/Yurufuwa/AgentCockpit
agp sim deploy                                      # app/test binaries + simulation device runtime を EC2 へ配置
...
```powershell
# Windows
agp device deploy                                # sensor_demo を adb push
adb shell
```
```bash
# RasPi5 (adb shell 内)
...
長期: agp sim run <target> と agp device run <target> が同じ manifest を読む
```
