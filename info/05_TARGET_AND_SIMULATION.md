...
| リモートコマンド実行 | `access/` のSSH・ADB command channel | ✅（中心） |
...
| 項目 | 根拠 | 影響 |
|---|---|---|
| **Sim 機能が完全に Linux/systemd 専用** | `commands/sim.py` が provider 選択を無視し常に `LinuxSystemdSimProvider` をハードコード | ESP32/Pico/STM32 では sim 機能が原理的に動かない |
...
| **ESP32 esptool provider** | `target_access/esp32_esptool.py` | ESP32 実機シリアル / firmware flash |
