...
現在は I2C/SPI を CUSE、GPIO を `gpio-sim` + GPIO chardev v2 で実現しています。EC2 側の runtime が実機と同じ `/dev/i2c-1`、`/dev/spidev0.0`、`/dev/gpiochip0` を用意するため、アプリは `~/sensor_demo` を直接起動します。runtime の設定・実行ファイルは `/etc/agentcockpit/hardware/`、`/usr/local/sbin/`、`/usr/local/lib/agentcockpit/`、`/run/agentcockpit/` に寄せ、アプリ本体だけを本番と同じユーザー領域の成果物として扱います。
...
[EC2 arm64 (Graviton)]
  ...
  sensor_demo (アプリケーション)
    ...
    │
    └─ bridge.py
         ├─ Unix socket /run/agentcockpit/hw_sim.sock (CUSE スタブ ⇔ bridge)
...
これで `gpio-sim` module が `/lib/modules/$(uname -r)/.../gpio-sim.ko.*` に入り、`agp sim env start` が `modprobe gpio-sim` できるようになります。
...
4. アプリ成果物や `agp sim env start` が作る runtime state は Terraform に持たせない。
...
1. `agp sim env start` で bridge と dummy device runtime を起動し、runtime host 上にテスト用 `/dev/*` を用意する。
...
GPIO dummy runtime だけを確認・更新したい場合は、full runtime の生コマンドを直接打たずに `agp sim gpio` を使います。

```bash
agp sim gpio plan --json
agp sim gpio install
agp sim gpio start
agp sim gpio status --json
agp sim gpio stop
```

`plan` はローカルの `hardware/gpio.csv` から生成される gpio-sim chip / line / label / service 配置の契約を表示します。`start` は `modprobe gpio-sim`、configfs chip 作成、必要な bind mount までを `agp-gpio-sim.service` 経由で行います。
...
`agp sim env start` が担当するのは、アプリではなく simulation runtime の起動です。bridge / CUSE / gpio-sim を個別の生コマンドで起動するのではなく、`agp` の操作単位を使います。
...
| sensor_demo が `/dev/gpiochip0: No such file` | simulation runtime 未起動 | `agp sim env start` 後に fake `/dev/gpiochip0` 起動状態を確認 |
| `Tap Card しても OLED に UID 出ない` | cuse_spi / bridge / system_on のいずれかが未接続 | `agp sim env diag --json`、`agp sim env log`、`sensor_demo` ログを確認 |
