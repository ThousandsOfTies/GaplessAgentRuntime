# LD_PRELOAD shim → CUSE 移行計画

このドキュメントは、現状 EC2 simulation runtime で使用している **GPIO / SPI の `LD_PRELOAD` shim** を、I2C と同じ **CUSE (Character device in Userspace) ベースの fake `/dev/*` runtime** に置き換えるための設計計画です。
...
1. アプリ起動コマンドから `LD_PRELOAD=...` を **完全に削除する**こと。EC2 でも RasPi5 でも `~/sensor_demo` を直接起動できる状態を到達点とする。
2. `LD_PRELOAD` 固有の制約（後述 §2）を取り除き、`mmap` ベースのドライバや別言語ランタイムからの利用にも耐える runtime にする。
3. `/dev/gpiochip0` / `/dev/spidev0.0` の **実機と同じパス・同じ ioctl ABI** を fake runtime で提供し、アプリ側のコード変更を不要にする。
4. CUSE 実装を AI エージェントが追従しやすい形（必要 ioctl だけを実装、ABI 変更は strace 差分で検知）に整理する。
...
アプリ起動時の現状コマンド（[AGENT.md](../AGENT.md) より）:
...
- `GPIO_GET_LINEINFO_IOCTL` (v1) は `EINVAL`。現行 `sensor_demo.c` は v1 (`GPIO_GET_LINEHANDLE_IOCTL`) を使うため、このままでは gpio-sim に移行できない。

したがって GPIO の次ステップは「CUSE GPIO を透明化」ではなく、**アプリ GPIO 呼び出しを GPIO chardev v2 に移す**、または libgpiod 2.x 相当の v2 経路に寄せること。これで `gpio-sim` の実 fd 払い出しを使える。
...
[03_SIMULATION_SETUP.md](03_SIMULATION_SETUP.md) §「全体構成」の「将来形」に対応する状態:
...
[EC2 arm64 (Graviton)]
  ...
  sensor_demo (アプリケーション、LD_PRELOAD 不要)
    ...
    │  ──→ cuse_gpio (CUSE で /dev/gpiochip0 を生成)
...
3 つの CUSE スタブ (`cuse_i2c`, `cuse_gpio`, `cuse_spi`) は次の構造を共有する。`cuse_i2c` の既存実装をテンプレートとして揃える。
...
### 4.2 GPIO CUSE (`cuse_gpio`)

**提供するノード**: `/dev/gpiochip0`
...
**実装ヒント**: `cuse_gpio` のライン要求 fd は CUSE が払い出せないため、main fd の中で内部仮想 fd を持ち、`GPIO_V2_GET_LINE_IOCTL` の返り値は擬似 fd 番号を返した上で、後続の ioctl(line_fd, ...) を main fd で受けて分岐する設計にする（`gpio_shim.so` の現行ロジックを移植）。これがやや非自明なため、AI が既存 shim ソースを参照して移植するのが効率的。
...
# /etc/systemd/system/agp-cuse-gpio.service
[Service]
ExecStart=/home/ubuntu/cuse_gpio -f --devname=gpiochip0
Restart=on-failure
...
| Step | 対象リポ | 内容 | 完了条件 |
|---|---|---|---|
| **S1** | agp-tools | `cuse_gpio` プロトタイプ実装。`GPIO_GET_CHIPINFO_IOCTL` + `GPIO_V2_LINE_SET_VALUES_IOCTL` のみ。LED18 トグルで panel が反応すること。 | `gpioset gpiochip0 18=1` が panel に反映 |
| **S2** | agp-tools | `cuse_gpio` の `GPIO_V2_LINE_GET_VALUES_IOCTL` + edge event 実装。Button17 押下が sensor_demo に届くこと。 | `agp sim` で sensor_demo の System ON/OFF 切替動作 |
| **S3** | AgentCockpit | `agp sim start` / `start.sh` から `LD_PRELOAD=gpio_shim.so` を削除。`cuse_gpio` を systemd で起動。`agp sim diag` に `cuse_gpio` 行を追加。 | sensor_demo を `LD_PRELOAD` なしで起動 → GPIO 動作 |
| **S4** | agp-tools | `cuse_spi` 実装。MFRC-522 register sim を `spi_shim.c` から移植。 | RFID Tap が OLED に UID 反映 |
| **S5** | AgentCockpit | `LD_PRELOAD=spi_shim.so` を削除。`cuse_spi` を systemd で起動。 | `LD_PRELOAD` 完全廃止、起動コマンド `~/sensor_demo` のみ |
| **S6** | AgentCockpit | docs (01, 03, 05, 06, AGENT.md, README.md) の `LD_PRELOAD` 言及を「移行完了」記述に更新。`gpio_shim.so` / `spi_shim.so` を artifact 一覧から削除。 | grep `LD_PRELOAD` で残るのは本ドキュメントのみ |
| **S7** | 両リポ | 旧 `gpio_shim` / `spi_shim` のソースを `legacy/` へ退避、または削除。 | リポからは消えるが、git history で参照可能 |
...
| Step | 対象リポ | 内容 | 完了条件 |
|---|---|---|---|
| **G1** | AgentCockpit | `agp sim gpio-sim-check --json` で EC2 の `gpio-sim` 対応を診断する。 | `ok: true`, `/dev/gpiochip1` が存在 |
| **G2** | AgentCockpit | `sensor_demo.c` の GPIO helper を v1 (`GPIO_GET_LINEHANDLE_IOCTL`) から v2 (`GPIO_V2_GET_LINE_IOCTL`) へ移行する。アプリは引き続き `/dev/gpiochip0` だけを開く。 | gpio-sim を環境側で `/dev/gpiochip0` として見せた状態で line request fd が返る |
| **G3** | agp-tools / AgentCockpit | bridge.py が gpio-sim の sysfs (`/sys/devices/platform/.../sim_gpioN/{pull,value}`) と同期する。 | panel の Button17 が `sensor_demo` に届き、LED18/24 が panel に反映 |
| **G4** | AgentCockpit | `agp sim start` に gpio-sim setup を組み込み、GPIO の `LD_PRELOAD` を削除する。 | `sensor_demo` が GPIO shim なしで起動 |
...
- EC2 で環境変数なしの `~/sensor_demo_v2` が GPIO setup 成功。
- `sim_gpio17/pull` を `pull-up` にすると `[btn] System ON` を確認。
- 実行中の `sim_gpio18/value` が `1` になることを確認。
