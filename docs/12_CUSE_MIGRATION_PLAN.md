...
main()
  ...
  ├─ Unix socket connect: /run/agentcockpit/hw_sim.sock (bridge.py)
...
EC2 上の simulation runtime は systemd で常駐させる形へ移行済み。`agp sim env start` は `hardware/*.csv` から GPIO line と I2C/SPI dev を読み、unit / helper script を配置して `agp-sim.target` を起動する。`agp sim env stop` は同じCSV由来の service 群を停止する。
...
agp sim env diag --json --host vibecode-graviton
```
...
現在の unit は `$HOME` 配下の `~/cuse_i2c`、`~/cuse_spi`、`~/web-bridge` 参照から移行済み。`agp sim env deploy` は旧 artifact manifest の `~/cuse_*` / `~/web-bridge` dest も互換マップし、runtime を `/etc/agentcockpit/hardware/`、`/usr/local/sbin/`、`/usr/local/lib/agentcockpit/`、`/run/agentcockpit/` へ配置する。

現在配置する主な runtime path:

```text
/etc/agentcockpit/hardware/*.csv
/usr/local/sbin/agp-gpio-sim-start
/usr/local/sbin/agp-gpio-sim-stop
/usr/local/sbin/agp-bridge-start
/usr/local/sbin/cuse_i2c
/usr/local/sbin/cuse_spi
/usr/local/lib/agentcockpit/web-bridge/
/run/agentcockpit/
/run/agentcockpit/hw_sim.sock
```
...
`agp sim env start / stop` は systemd unit を `systemctl start/stop` する形に切り替え済み（旧 `setsid nohup` ベースは fallback 掃除のみ残す）。
...
agp-tools リポジトリと AgentCockpit リポジトリで分担。各ステップの完了は `agp sim env diag --json` の `ok` と、`agp sim ui button/rfid`、`agp sim env status --json`、`agp sim env log` による実機能確認で判定する。

| Step | 対象リポ | 内容 | 完了条件 |
|---|---|---|---|
| **S1/S2** | agp-tools | `cuse_gpio` プロトタイプ調査。fd 払い出し問題を確認。 | `gpio-sim` ルートへ方針転換 |
| **S3** | AgentCockpit | `agp sim env start` / 起動手順から `LD_PRELOAD=gpio_shim.so` を削除。GPIO は `gpio-sim` へ移行。 | sensor_demo を `LD_PRELOAD` なしで起動 → GPIO 動作 |
...
| Step | 対象リポ | 内容 | 完了条件 |
|---|---|---|---|
| **G1** | AgentCockpit | `agp sim env gpio-sim-check --json` で EC2 の `gpio-sim` 対応を診断する。 | 完了 |
...
| **G4** | AgentCockpit | `agp sim env start` に gpio-sim setup を組み込み、GPIO の `LD_PRELOAD` を削除する。 | 完了 |
...
- `agp sim env start` で gpio-sim chip を作成し、必要なら `/dev/gpiochip1` を `/dev/gpiochip0` に bind mount する。
- EC2 で環境変数なしの `~/sensor_demo` が GPIO setup 成功。
- `agp sim ui button press 17` で `[btn] System ON/OFF` を確認。
- 実行中の `sim_gpio18/value` と panel state の LED18 が同期することを確認。
- `agp sim ui rfid tap 04:AB:CD:EF:01:23` で UID ログを確認し、`agp sim ui rfid remove` で `present=0` に復帰。
...
- `agp sim env diag` の出力フォーマットを CUSE 統一にあわせて再設計するか（[07_AI_AGENT_OPERATIONS.md](07_AI_AGENT_OPERATIONS.md) の `--json` 化方針と合流させると良い）。
