...
### Web ブリッジ + パネル (`agp-tools/cuse-stubs/web-bridge/`)
- `bridge.py` — Unix socket ↔ WebSocket ↔ HTTP ブリッジ
- `panel/` — HTML/CSS/JS の Virtual Hardware Panel

### 開発インフラ
- 旧 Windows PowerShell EC2 helper → **`agp sim boot/shutdown/status`** に移植。WSL2 の AWS CLI で EC2 起動・停止・SSH config 自動更新
- 旧 Windows PowerShell RasPi helper → **`agp native sync`** に移植。Codespaces から artifact bundle を取得し RasPi5 に ADB push / SSH 配送
...
- [x] systemd サービス化による simulation runtime 起動 — 2026-06-05。EC2 で `agp-sim.target`、`agp-gpio-sim.service`、`agp-bridge.service`、`agp-cuse-i2c@i2c-1.service`、`agp-cuse-spi@spidev0.0.service` の active を確認
- [x] SPI/GPIO の fake device runtime への移行（設計: [12_CUSE_MIGRATION_PLAN.md](12_CUSE_MIGRATION_PLAN.md)）
- [ ] `agp sim run <target>` / `agp native run <target>` の共通 manifest 化
