...
- 旧 Windows PowerShell RasPi helper → **`agp target sync`** に移植。Codespaces から artifact bundle を取得し RasPi5 に ADB push / SSH 配送
...
- [ ] `agp sim run <target>` / `agp target run <target>` の共通 manifest 化
