...
| コマンド | 実行場所 | 目的 | 主な処理 | 備考 |
|---|---|---|---|---|
| `agp native deploy` | AgentCockpit (venv) | native runtime へ成果物を配置 | AGP artifact bundle の `deploy.native.files` に従って `adb push` で接続先へ転送 | dummy device runtime は配置しない |
