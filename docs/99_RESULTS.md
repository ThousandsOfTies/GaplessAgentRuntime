...
この PoC で見えた最大の価値は、「製品には見えないが、壊れると開発が止まる」互換 runtime を AI が作り切れる見通しが立ったことです（背景は [info/01_INDUSTRY_TRENDS.md](../info/01_INDUSTRY_TRENDS.md)）。
...
### CUSE スタブ (`gar-tools/targets/linux-device/runtime/`)
...
### Web ブリッジ + パネル (`gar-tools/targets/linux-device/runtime/web-bridge/`)
...
7. **AI コーディングの適所** — 実機互換 runtime は、`strace` による ABI 抽出、stub 実装、ログ確認、回帰テスト追加といった、AI が得意な地味で構造化された作業の集まりでした（差別化の背景は [info/01_INDUSTRY_TRENDS.md](../info/01_INDUSTRY_TRENDS.md)）。
