...
この移行は、人間が手で保守するには地味で採算が合いにくい作業です。一方で、AI エージェントは `strace`、既存 shim、実機ログ、テストバイナリを材料に、プロジェクトが実際に使う ioctl ABI だけを抽出して実装できます。AgentCockpit は、この「人手では腐りがちな互換 runtime」を AI が継続的に維持するための開発コックピットです。
