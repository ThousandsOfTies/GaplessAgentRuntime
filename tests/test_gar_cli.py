...
class AgpCliTest(unittest.TestCase):
    ...
    def test_sim_status_json_returns_bridge_state(self) -> None:
        with mock.patch("scripts.gar_lib.sim.linux.LinuxSystemdSimProvider.panel", return_value=0) as panel:
    ...
    def test_deploy_target_pushes_sensor_demo_with_adb(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("scripts.gar_lib._deploy.subprocess.run", side_effect=run_side_effect) as run:
    ...
    def test_sim_vm_commands_are_available_from_cli(self) -> None:
        ...
        for argv, ec2_command in cases:
            with self.subTest(argv=argv):
                with mock.patch("scripts.gar_lib.cli.run_ec2_command", return_value=0) as run_ec2:
                    result = main(argv)
