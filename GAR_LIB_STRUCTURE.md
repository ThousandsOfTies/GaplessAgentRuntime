...
```text
gar_lib/
├─ commands/       CLI境界の表示・復旧と補助command
...
├─ vscode/         VS Code 統合
├─ application.py  workspace・environment・artifactの協調
├─ composition.py  config依存の具体オブジェクト接続
├─ hardware.py     hardware定義のRepositoryとCSV実装
...
| フォルダ | 役割 |
|---|---|
| `commands/` | setup・code・infra等の補助commandと、Application結果の表示・接続復旧を担当するCLI境界。 |
...
| `cli_tmp.py` | workspaceベースの`sim` / `target`操作だけを並べた、次期CLI構造の実行可能な参照実装。 |
| `application.py` | `Workspace`、`BuildEnvironment`、`Artifact`、`SimulationEnvironment`、`TargetEnvironment`の協調順序を記述する。 |
| `composition.py` | configを読むRegistry / Resolver / Storeの具体実装を生成してApplicationへ接続する。 |
| `hardware.py` | Applicationが利用するhardware定義RepositoryとCSVによる具体実装。CLI表示には依存しない。 |
...
- `application.py` はworkspaceから必要なenvironmentを解決し、ユースケースの順序だけを決める。
...
- AWS認証やSSH接続に失敗した場合の利用者誘導は、environmentではなく `commands/application.py` の共通recovery経路がTerminal Bridgeへ渡す。
...
- `application.py` はbuild・artifact選択・deployの順序を扱い、`commands/application.py` が表示と接続復旧を扱う。
...
commands             # CLI表示と復旧方針
  ↓
application          # ユースケースと高水準オブジェクトの協調
  ↓
