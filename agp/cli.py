...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    terminal_run_parser.add_argument(
        ...
        help="実行するコマンド。例: agp terminal run -- agp init",
    )

    args = parser.parse_args(argv)

    if args.command == "init":
        return run_init(no_install=args.no_install)
...
def run_init(no_install: bool = False) -> int:
...
def print_terminal_bridge_status(*, offer_install: bool) -> None:
    ...
    if not offer_install or not sys.stdin.isatty():
        print(f"     {style('導入するには make install-vscode-extension を実行してください。', DIM)}")
        return
    ...
    if answer not in ("", "y", "yes"):
        print(f"     {style('あとで make install-vscode-extension で導入できます。', DIM)}")
        return
