"""`agp usb` subcommand: USB-C passthrough to WSL2 via usbipd-win.

WSL2 から Windows interop で ``usbipd.exe`` を呼び出し、USB-C 実機（ADB）を
WSL2 に attach する。busid は自動検出し、一度確定したものは ``.agp/config.json``
に記憶するので、2 回目以降は ``agp usb attach`` だけで済む。
...
def _resolve_target(
    ...
    *,
    ...
    if busid:
        for device in devices:
            if device.busid == busid:
                return device
        print(f"agp usb: busid {busid} のデバイスが見つかりません。", file=sys.stderr)
        return None
    ...
    if len(candidates) == 1:
        return candidates[0]
    if len(candidates) > 1:
        print("agp usb: ADB 実機の候補が複数あります。--busid で指定してください:", file=sys.stderr)
        for device in candidates:
            print(f"  {_format_device(device)}", file=sys.stderr)
        return None

    print(
        "agp usb: ADB 実機を自動検出できませんでした。`agp usb list` で busid を確認し "
        "`agp usb attach --busid <busid>` を実行してください。",
        file=sys.stderr,
    )
    return None
...
def _print_bind_hint(busid: str) -> None:
    print(
        "agp usb: このデバイスはまだ share されていません。\n"
...
def run_usb_command(
    ...
    *,
    ...
    if _usbipd_executable() is None:
        print(
            "agp usb: usbipd.exe が見つかりません。Windows 側に usbipd-win を install してください。\n"
    ...
    try:
        ...
        print("agp usb: usbipd.exe の実行に失敗しました。", file=sys.stderr)
        return 1
    ...
    if command == "attach":
        ...
        if target.is_attached:
            print(f"agp usb: 既に attach 済みです: {_format_device(target)}")
            if remember:
        ...
        print(f"agp usb: attach しました: {target.busid} ({target.description})")
        if remember:
            _remember(config, target.busid)
        return 0
    ...
    if command == "detach":
        ...
        if result.returncode != 0:
            ...
            return result.returncode
        print(f"agp usb: detach しました: {target.busid}")
        return 0
