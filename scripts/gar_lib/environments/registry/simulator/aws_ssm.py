...
class AwsSsmEnvironment(EnvironmentSetupOption):
    ...
    def install_dependencies(cls, missing: list[str]) -> int:
        ...
        with tempfile.TemporaryDirectory(prefix="gar-aws-ssm-") as tmp:
            work_dir = Path(tmp)
...
def _manual_install_commands(missing: list[str]) -> list[str]:
    ...
    if "aws" in missing:
        commands.extend(
            [
                f'curl "https://awscli.amazonaws.com/awscli-exe-linux-{arch}.zip" -o "$GAR_TMP/awscliv2.zip"',
                'unzip -q "$GAR_TMP/awscliv2.zip" -d "$GAR_TMP"',
                'sudo "$GAR_TMP/aws/install"',
            ]
        )
    ...
    if "session-manager-plugin" in missing:
        ...
        commands.extend(
            [
                ...
                '"https://s3.amazonaws.com/session-manager-downloads/plugin/latest/'
                f'{deb_arch}/session-manager-plugin.deb" '
                '-o "$GAR_TMP/session-manager-plugin.deb"',
                'sudo dpkg -i "$GAR_TMP/session-manager-plugin.deb"',
            ]
        )

    commands.append('rm -rf "$GAR_TMP"')
    return commands
