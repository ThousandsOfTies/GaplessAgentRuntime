...
- **機械可読モードを使う** — AI が状態を判断するときは `--json` を付ける（例: `agp sim env diag --json`）。人間向けの整形出力をパースしない。
...
| **WSL** | コントロールプレーン | `agp` 発信、成果物中継、deploy | ターゲット用バイナリのビルド |
...
# ※ 上記は手動参照用。通常は `agp sim env start` が bridge/cuse_i2c/cuse_spi を一括起動する
~/sensor_demo
```
...
```bash
agp sim boot       # 起動 + SSH config 自動更新（--pull で git pull も実行）
agp sim shutdown   # 停止
agp sim status  # 状態確認
```
